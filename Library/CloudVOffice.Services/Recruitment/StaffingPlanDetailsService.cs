...
namespace CloudVOffice.Services.Recruitment
{
    public class StaffingPlanDetailsService : IStaffingPlanDetailsService
    {
        ...
        public MessageEnum StaffingPlanDetailsUpdate(StaffingPlanDetailsDTO staffingPlanDetailsDTO)
        {
            try
            {
                var StaffingPlanDetails = _Context.StaffingPlanDetails.Where(x => x.StaffingPlanDetailsId != staffingPlanDetailsDTO.StaffingPlanDetailsId && x.Deleted == false).FirstOrDefault();
                if (StaffingPlanDetails == null)
                {
                    var a = _Context.StaffingPlanDetails.Where(x => x.StaffingPlanDetailsId == staffingPlanDetailsDTO.StaffingPlanDetailsId).FirstOrDefault();
                    if (a != null)
                    {
