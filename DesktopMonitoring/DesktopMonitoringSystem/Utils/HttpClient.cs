...
namespace DesktopMonitoringSystem.Utils
{
    public class HttpClientRq
    {
        ...
        public static Task<HttpResponseMessage> PostRequest(string URI, string parameterValues)
        {
            using (var client = new HttpClient())
            {
                ...
                var response = client.PostAsync(URI, c);
                response.Wait();
                return response;
            }
        }

        public static async Task<HttpResponseMessage> UploadFilesAsync(string URI, string path)
        {
